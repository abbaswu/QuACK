...
runtime_term_sharing_equivalent_set_top_nodes_to_runtime_term_sets: defaultdict[
    _ast.AST, set[RuntimeTerm]
] = defaultdict(set)
node_disjoint_set_lock: asyncio.Lock = asyncio.Lock()

node_containment_graph: nx.DiGraph = nx.DiGraph()
node_containment_graph_lock: asyncio.Lock = asyncio.Lock()
...
async def _add_runtime_terms_to_node_only(
        to_node: _ast.AST,
...
async def handle_attribute_access_propagation_task(
    ...
    import datetime
    if runtime_term is datetime and attribute_name == 'timedelta':
        print()
...
async def add_containment_relation(
        superset_node: _ast.AST,
        subset_node: _ast.AST
):
    if superset_node in debugging_singleton.node_set:
        print()
    async with node_containment_graph_lock:
        if superset_node != subset_node:
            node_containment_graph.add_edge(superset_node, subset_node)


async def add_two_way_containment_relation(
        first_node: _ast.AST,
        second_node: _ast.AST
):
    await add_containment_relation(
        superset_node=first_node,
        subset_node=second_node
    )

    await add_containment_relation(
        superset_node=second_node,
        subset_node=first_node
    )
...
async def set_equivalent(node_set: typing.AbstractSet[_ast.AST], propagate_runtime_terms: bool, /):
    ...
    while pairwise_queue:
        ...
        if propagate_runtime_terms:
            ...
            for runtime_term_sharing_target_equivalent_set_node in runtime_term_sharing_target_equivalent_set:
                await _add_runtime_terms_to_node_only(
